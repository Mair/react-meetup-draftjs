...
export class ComplexDecorator extends React.Component<null, ComplexDecoratorState> {
  constructor(props: any) {
    ...
    const stockItemComponent = (props: { decoratedText: string; entityKey: string; children: any }) => {
      if (props.entityKey) {
        ...
        return <EntityStock stockItem={data}>{props.children}</EntityStock>;
      }
      const stockName = props.decoratedText.replace(" stock:", "");
      return (
        <StockSuggestion stockSuggestion={stockName} stockClicked={this.stockItemClicked}>
          {props.children}
        </StockSuggestion>
      );
    };
  ...
  stockItemClicked = (stockItem: StockItemDef) => {
    ...
    const newEditorState = Draft.EditorState.push(this.state.editorState, newContnentState, "insert-characters");
...
const StockItem = (props: StockItemDef) => (
